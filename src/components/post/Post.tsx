...
export default function Post({ post, userId }: PostProps) {
  ...
  return (
    <div className="bg-dark-3 p-4 rounded-2xl my-6 shadow-md">
      <div className="flex gap-3 items-center">
        <div className="relative w-10 h-10">
          <Image src={avatar} alt="profile-pic" fill className="object-cover rounded-full border-2 border-dark-4" />
        </div>
      ...
      <Link href={`/post/${post.id}`}>
        {post.text && <p className="py-4 text-gray-200 text-sm leading-relaxed">{post.text}</p>}
        {post.image && (
      ...
      <Suspense fallback={<div className="h-10 animate-pulse bg-dark-4 rounded-md mt-4" />}>
        <PostActions postId={post.id} creatorId={post.author.id} userId={userId} postViewPage={false} />
      </Suspense>
    </div>
  );
}
